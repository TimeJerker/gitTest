...
std::map <double, std::map <int, std::vector <double> > > mp;
...
int main(){
    ...
    if ((int)alpha == alpha) {
        for (int i = 1; i <= alpha; i++) {
            ...
            mp[x][i].push_back(equation(x,i));
            mp[x][i].push_back(sum);
            }else{
                sum += mp[x][i][0];
                if(mp[x].count(i+1)){
                epsilon = abs(mp[x][i+1][0] / mp[x][i][1]);
                }else{
                    epsilon = abs(equation(x, i + 1) / mp[x][i][1]);
                }
                std::cout << "n = " << i << " An = " << mp[x][i][0] << " sum = " << mp[x][i][1] << " EPSILONn = " << epsilon << " Cached" <<'\n';
